...
Create a Contact in Xero from a Contact in Suite
...
if (isset($_REQUEST['wipe'])) {
	...
	if ($account->id == '') {
				...
				if ($XeroOAuth->response['code'] == 200) {
					...
					if (count($contact->Contacts[0]) > 0) {
						...
						if (isset($_REQUEST['hook_called']) && $_REQUEST['hook_called']) {
							...
							$this->updateCstmTable('contacts_cstm', $fieldsToSave); // on creating facing issue on record save fields
						} else {
						...
						if (isset($_REQUEST['hook_called']) && $_REQUEST['hook_called']) {
							$this->writeLogger('The contact record has been successfully added to Xero.');
						} else {
							echo ("<SCRIPT LANGUAGE='JavaScript'>
										window.alert('The contact record has been successfully added to Xero.')
										window.location.href='index.php?module=Contacts&action=DetailView&record=" . $contactobj->id . "';
								</SCRIPT>");
						}
					}
		...
		if ($account->xero_id_c != '') {
			...
			//code for duplication of Contact check
				...
				if (count($duplicatecontact->Contacts[0] > 0) && ($XeroContactobj->ContactID != '')) {
					...
					if ($XeroOAuth->response['code'] == 200) {
						...
						if (count($contact->Contacts[0]) > 0) {
							...
							if (isset($_REQUEST['hook_called']) && $_REQUEST['hook_called']) {
								$this->writeLogger('Contact details have been sucessfully added to Xero.');
							} else {
								echo ("<SCRIPT LANGUAGE='JavaScript'>
								window.alert('Contact details have been sucessfully added to Xero.')
								window.location.href='index.php?module=Contacts&action=DetailView&record=" . $contactobj->id . "';
								</SCRIPT>");
							}
						}
