...
Get all Xero invoices from the SuiteCRM Contacts and Accounts Actions menu
...
if (isset($_REQUEST['wipe'])) {
	...
	} else if ($_REQUEST['module'] == 'Contacts' &&  $_REQUEST['xeroID'] != '') {
		$xeroID = $_REQUEST['xeroID'];
	...
	if ($XeroOAuth->response['code'] == 200) {
		...
		if (isset($Invoices->Invoices)) {
			foreach ($Invoices->Invoices->Invoice as $Invoice) {
				...
				$Invoicebean->dtime_synched_c = $CurrenrDateTime;
				$Invoicebean->save();
				//Entry for Group
	...
	(id,name,date_entered,date_modified ,total_amt,tax_amount,subtotal_amount,total_amount,parent_type,parent_id,number) VALUE ('$oid','Xero Invoices',NOW(),NOW(),'$SubTotal','$TotalTax','$SubTotal','$Total','AOS_Invoices','" . $Invoicebean->id . "','1')";
			...
			window.alert('All Invoices for this Account have been successfully updated.')
			window.location.href='index.php?module=Accounts&action=DetailView&record=" . $account->id . "';
			</SCRIPT>");
			} else if ($_REQUEST['module'] == 'Contacts') {
				echo ("<SCRIPT LANGUAGE='JavaScript'>
			window.alert('All Invoices for this Contact have been successfully updated.')
			window.location.href='index.php?module=Contacts&action=DetailView&record=" . $contact->id . "';
			</SCRIPT>");
			}
		} else {
			if ($_REQUEST['module'] == 'Accounts') {
				echo ("<SCRIPT LANGUAGE='JavaScript'>
			window.alert('There are no invoices for this Account in Xero.')
			window.location.href='index.php?module=Accounts&action=DetailView&record=" . $account->id . "';
			</SCRIPT>");
			} else if ($_REQUEST['module'] == 'Contacts') {
				echo ("<SCRIPT LANGUAGE='JavaScript'>
			window.alert('There are no invoices for this Contact in Xero.')
			window.location.href='index.php?module=Contacts&action=DetailView&record=" . $contact->id . "';
			</SCRIPT>");
			}
		}
