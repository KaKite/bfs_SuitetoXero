...
Download ALL invoices from Xero from the related Account in SuiteCRM
...
if (isset($_REQUEST['wipe'])) {
	...
	$count = '';
	$countNoXero = '';
	// page counter for xero records > 100
	$xero_records = 0;
	$page_number = 0;
	$invoice_saved = 0;
	...
	(id,name,date_entered,date_modified ,total_amt,tax_amount,subtotal_amount,total_amount,parent_type,parent_id,number) VALUE ('$oid','Xero Invoices',NOW(),NOW(),'$SubTotal','$TotalTax','$SubTotal','$Total','AOS_Invoices','" . $Invoicebean->id . "','1')";
		...
		$xero_records = $xero_records + 1;
	}
	if ($countNoXero != 0 && $_REQUEST['module'] == 'Accounts') {
		echo ("<SCRIPT LANGUAGE='JavaScript'>
	  alert('$count Account records have been checked for invoices in Xero.\\n\\n$countNoXero records in Suite had NO Xero ID data and were skipped. If you want to check them\\nin future you need to run the Send to Xero function first\\n\\nONLY those records that have an existing Xero ID are checked. $count_checked Accounts were checked,\\nwith $invoice_saved invoices updated or downloaded $xero_records');
				window.location.href='index.php?module=Accounts&action=index';
		</SCRIPT>");
	} else {
		echo ("<SCRIPT LANGUAGE='JavaScript'>
	alert('$count Account records were checked for invoices in Xero and their data updated to Suite.');
	window.location.href='index.php?module=Accounts&action=index';
	</SCRIPT>");
	}
}
