...
class XeroOAuth
{
	...
	function __construct($config)
	{
		...
		*/
		...
		$this->_xero_curl_options = array( // you probably don't want to change any of these curl values
			...
			// include ca-bundle.crt from http://curl.haxx.se/ca/cacert.pem // removed for Oauth2.0
	...
	function request($method, $url, $params = array(), $xml = "", $format = 'xml')
	{
		...
		if (!empty($this->config['access_token'])) { // added for Oauth2.0 condition for new header in request
			$this->headers['Authorization'] = 'Bearer ' . $this->config['access_token'];
		}

		if (!empty($this->config['tenantId'])) { // added for Oauth2.0 condition for new header in request
			$this->headers['Xero-tenant-id'] = $this->config['tenantId'];
		}
		...
		if ($this->response['code'] == 401 && isset($this->config['session_handle'])) {
			if ((strpos($this->response['response'], "TokenExpired") !== false)) {
	...
	function url($request, $api = "core")
	{
		if ($request == "RequestToken") {
			...
			$this->config['host'] = $this->config['site'] . '/oauth/';
		} elseif ($request == "connections") { // added for Oauth2.0 new conditions by Rupendra
			$this->config['host'] = $this->config['site'] . '/';
		} else {
			if (isset($api)) {
				if ($api == "core") {
					$api_stem = "api.xro";
	...
	@ request new token and refresh token
	@ params $redirect_url/null
	@ params $code/null
	@ params $refresh/null
	@ return code
	*/
	function requestToken($redirect_url = null, $code = null, $refresh = false)
	{
		$params = [
		...
		if ($refresh) {
			...
			$params['redirect_uri']    = 'http://localhost/suitecrm/index.php?entryPoint=XeroCallBack';
		} else {
		...
		// store the response
		$this->response['code'] = $code;
		$this->response['response'] = (array) (json_decode($response));
		$_SESSION['access_token'] = $this->response['response']['access_token']; // added access_token in session added row by Rupendra
		$this->response['info'] = $info;
		return $code;
	}
}
